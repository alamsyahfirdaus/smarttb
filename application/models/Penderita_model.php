...
class Penderita_model extends CI_Model {

	private $table 			= 'penderita';
	private $col_order 		= ['id_penderita', 'nik', 'full_name', 'umur', 'berat_badan', 'tinggi_badan', NULL, NULL];
	...
	private function _get_button($field)
	{
		...
		$button		.= '<a class="dropdown-item" href="javascript:void(0)" onclick="reminder_obat(' . "'" . md5($field->id_penderita) . "'" . ')">Reminder Obat</a>';
	...
	{
		$this->_join();
		return $this->db->get_where($this->table, ['md5(penderita.id_penderita)' => $id])->row();
	}

	private function _reminder_obat($user_id)
	{
		$query 	= $this->db->get_where('pengingat_obat', ['user_id' => $user_id])->row();
		$status	= @$query->status == 1 ? 'Proses Pengobatan' : 'Selesai Pengobatan';
		return @$query ? $status : 'Belum Diatur';
	}

}

/* End of file Penderita_model.php */
/* Location: ./application/models/Penderita_model.php */