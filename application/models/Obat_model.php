...
class Obat_model extends CI_Model {
	...
	private $table 			= 'obat';
	private $col_order 		= ['id_obat', 'nama_obat', NULL];
	private $col_search 	= ['id_obat', 'nama_obat'];
	private $order_by		= ['id_obat ' => 'DESC'];
	...
	private function _get_button($field)
	{
		...
		$button		.= '<a class="dropdown-item" href="javascript:void(0)" onclick="edit_data(' . "'" . md5($field->id_obat) . "'" . ', ' . "'". $field->nama_obat . "'" . ')">Edit</a>';
		$button		.= '<div class="dropdown-divider"></div>';
		$button		.= '<a class="dropdown-item" href="javascript:void(0)" onclick="delete_data(' . "'" . md5($field->id_obat) . "'" . ')">Hapus</a>';
		$button		.= '</div>';
		$button		.= '</button>';
		$button		.= '</div>';
